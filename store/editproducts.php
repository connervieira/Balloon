...
if (file_exists("./config.php")) { // Find 'config.php' and import it
    ...
    echo "<p style='margin:15%;color:red;'>Error: The configuration file (balloon/store/config.php) couldn't be located! This probably isn't a problem with your configuration, and is a bug with Balloon itself. You may want to contact V0LT over this issue: <a href='mailto:cvieira@v0lttech.com'>mailto:cvieira@v0lttech.com</a></p>";
    exit();
}

if ($username != $admin_account) { // Check to make sure the current user is an admin before loading the page.
    echo "<p>Error: You are not authorized to be here! If you do actually have permission to edit products, please ensure you are signed in with the correct account, as specified by <b>\$admin_account</b> in <b>balloon/store/config.php</b></p>"; // Display an error message to the user.
...
<html lang="en" style="background:<?php echo $background_gradient_bottom; ?>;">
    <head>
	    ...
	    <title>Balloon - Edit Products</title>
                            ...
                            if ($_GET["producttodelete"] != null and $_GET["producttodelete"] != "") { // If the user has pressed the delete button on a product...
                                ...
                                <div style='width:100%;text-align:center;'>
                                    ...
                                    <form style='color:white;' method='POST'>
                                        ...
                                        Price (XMR): <input type='number' name='price' step='0.000001' required><br>
                                ...
                                foreach ($productArray[$store_id] as $key => $element) {
                                    ...
                                    echo "Price (XMR): <input type='number' name='price' step='0.000001' value='" . $element['price'] . "' required><br>";
                                ...
                                if ($price == "" or $price == null) { // Make sure the user has entered the price of this product
                                    echo "<p style='color:#ff9999'>Error: You haven't entered a price for this product! A price is required so that Balloon knows how much to charge customers for this product.</p>";
                                    exit();
                                }
