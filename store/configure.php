...
if (file_exists("./config.php")) { // Find 'config.php' and import it
    ...
    echo "<p style='margin:15%;color:red;'>Error: The configuration file (balloon/store/config.php) couldn't be located! This probably isn't a problem with your configuration, and is a bug with Balloon itself. You may want to contact V0LT over this issue: <a href='mailto:cvieira@v0lttech.com'>cvieira@v0lttech.com</a></p>";
    exit();
}
		...
		<div class="projects-clean" style="background:linear-gradient(0deg, <?php echo $background_gradient_bottom; ?>, <?php echo $background_gradient_top; ?>);color:#111111;">
		    <div class="container" style="padding-top:100px;">
	 	        <main>
    			    <div class="row projects" style="padding-left:5%;padding-right:5%;">
                        ...
                        <?php
                            ...
                            if ($_POST["store_id"] != null and $_POST["store_id"] != "") {
                                // Run basic input validation to catch simple mistakes in submitted data. It should be made clear that this is not designed to prevent attackers from compromising Balloon in the even that someone gains access to the admin account.
                                ...
                                if ($_POST["display_balloon_logo"] == "on") { $configurationArray[$store_id]["display_balloon_logo"] = true; $display_balloon_logo = true; } else { $configurationArray[$store_id]["display_balloon_logo"] = false; $display_balloon_logo = false; }
                                $configurationArray[$store_id]["display_monero_accepted_icon"] = $_POST["display_monero_accepted_icon"]; $display_monero_accepted_icon = $_POST["display_monero_accepted_icon"];
                                ...
                                echo "<p style='font-size:40px;color:#aaffaa;text-align:center;width:100%;margin-bottom:100px;'>Successfully updated Balloon configuration</p>";
                            }
                            ...
                            if ($display_balloon_logo == true) { echo "<label for='display_balloon_logo'>Display Balloon Logo: </label><input type='checkbox' name='display_balloon_logo' checked><br>"; } else { echo "<label for='display_balloon_logo'>Display Balloon Logo: </label><input type='checkbox' name='display_balloon_logo'><br>"; }
                            echo "    <label for='display_monero_accepted_icon'>'Monero Accepted' Icon Level: <input type='number' step='1' min='0' max='3' name='display_monero_accepted_icon' value='" . $display_monero_accepted_icon . "'><br>";
                    ...
                    <?php
                    // Display credits to V0LT depending on the current Balloon configuration.
                    if ($v0lt_credit == 2) {
                        echo '<p class="description" style="font-size:15px;color:#cccccc;margin-top:30px;margin-bottom:100px;text-align:center;"><a href="https://v0lttech.com" style="text-decoration:underline;color:inherit;">Balloon - Made By V0LT</p>';
                    } else if ($v0lt_credit == 3) {
                        echo '<div style="position:fixed;right:0;bottom:0;margin-right:10px;margin-bottom:10px;padding-left:5px;padding-right:5px;border-radius:5px;background:rgba(0, 0, 0, 0.75);"><p style="margin-bottom:7px;margin-top:7px;"><a href="https://v0lttech.com/" style="text-decoration:underline;color:white;">Balloon - Made by V0LT</a></p></div>';
                    }
                    ?>
                </main>
            </div>
