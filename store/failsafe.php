...
if (strlen($xPub) <= 10) {
    echo "<p style='margin:15%;color:red;'>Error: The owner of this store hasn't properly configured their Master Public Key. Without it, Balloon can't generate the wallet addresses required to accept payment! You may want to contact customer support to make the store owner aware of this issue: <a href='mailto:";
    echo $support_email;
    echo "'>";
    echo $support_email;
    echo "</a></p>";
...
if (isset($authenticationArray[$admin_account]) == false) {
    echo "<p style='margin:15%;color:yellow;'>Notice: The current admin account set in the configuration is named '" . $admin_account . "'. However, no such account exists in the authentication database. This means that anyone can sign up for an account with this name, and have full control over this Balloon instance. If you are the owner of this Balloon instance, you should sign up for an account with this name so that you can access admin controls and prevent others from being able to take over.</p>";
}
